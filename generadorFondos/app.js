...
const fallbackTrack = {
  title: "Diego San Marcos - Abre Tus Manos",
  src: "Diego San Marcos - Abre Tus Manos.mp3"
};
...
async function loadPlaylist() {
  ...
  buildPlaylist();
  if (playlist.length) {
    audio.src = resolveSrc(playlist[0].src);
  }
}
...
playBtn.addEventListener("click", () => {
  ...
  if (!audio.src && playlist.length) {
    audio.src = resolveSrc(playlist[currentTrackIndex].src);
  }
  audio.play().catch(() => {
    updateStatus();
  });
