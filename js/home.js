...
import { getHomeProjectTitle, hexToRgb } from './helpers.js';
import { getInitialModeFromURL } from './router.js';
import { showView } from './project.js';
import { getMenuLabels, openMenu } from './menu.js';
...
function updateHomeSEO() {
    ...
    const copy = langCopy[state.currentMode] || langCopy.portfolio;

    const canonical = new URL(window.location.pathname, window.location.origin).toString();
    const bgFile = state.appData?.modes?.[state.currentMode]?.background;
...
function enterHomeView() {
    ...
    state.currentMode = getInitialModeFromURL();
    showView('home');
    setHomeBackground(state.currentMode);
...
function renderProjects() {
    ...
    const modeConfig = state.appData.modes[state.currentMode] || {};
    const categories = modeConfig.categories || [];
    const filteredProjects = state.appData.projects.filter(p => categories.includes(p.tipo) && p.visible !== false);
...
function createProjectCard(project) {
    ...
    const colorValue = state.appData.typeColors[project.tipo] || '#808080';
    const colorHex = colorValue.startsWith('#') ? colorValue : (state.coloresData.colores[colorValue] || '#808080');
...
function setupSpecialTile(el, tag) {
    const colors = state.appData.modes[state.currentMode].tileColor;
...
function createSwitchTile() {
    const nextMode = state.currentMode === 'portfolio' ? 'personal' : 'portfolio';
    const colors = state.appData.modes[state.currentMode].tileColor;
    ...
    const icon = document.createElement('img');
    icon.src = assetUrl(`data/icons/${state.appData.modes[state.currentMode].switchIcon}`);
    icon.alt = 'switch';
...
function playIrisTransition(nextMode) {
    const color = state.appData.modes[nextMode].tileColor.bg;
